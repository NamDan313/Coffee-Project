...
public class Jp_QLThucDon extends javax.swing.JPanel {
    ...
    private GetDataThucDon menu=new GetDataThucDon();
    private GetDataLoai search=new GetDataLoai();
    /**
    ...
    public Jp_QLThucDon() {
        ...
        initComponents();
        Fillcbb();
        FillTable(null);// fill thông tin tất cả món
    }
    public void Fillcbb() {
        //Lấy thông tin nhóm móm từ mysql thông qua pakage DAL
        Vector Vcbb = search.GetNhomMon();

        if (Vcbb != null) {// định nghĩa danh sách hiển thị dữ liệu
            DefaultComboBoxModel cbbmodel = new DefaultComboBoxModel(Vcbb);// 
            cbbNhomMon.setModel(cbbmodel);// giống như setmodel của table
        } else {
    ...
    public void FillTable(String nhom) {
        // lấy thông tin món từ mysql lên table
        Vector<ThucDon> arrTable = menu.GetThucDon(nhom);
    ...
    private void initComponents() {
        ...
        bntSuaMon.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Interface/Images/document_edit.png"))); // NOI18N
    ...
    private void bntThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntThemActionPerformed
        Dl_Them_ThucDon td = new Dl_Them_ThucDon(Run.homepage);
        td.setVisible(true);
    ...
    private void bntSuaMonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntSuaMonActionPerformed
        ...
        if(select<0){
            ...
            Dl_Sua_ThucDon sua = new Dl_Sua_ThucDon(Run.homepage, mamon);
            sua.setVisible(true);
        }
    ...
    private void bntXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntXoaActionPerformed
        ...
        if (selectedRow <= 0) {
            ...
            String sp = "";
                ...
                String ma = (String) tbBan.getValueAt(selectedRow, 0);
                String tenban = (String) tbBan.getValueAt(selectedRow, 1);

                sp += tenban + "\n";
            int qs;
            qs = JOptionPane.showConfirmDialog(null, "Xóa món: \n " + sp, "Xóa món", JOptionPane.YES_NO_OPTION);
            if (qs == JOptionPane.YES_OPTION) {
                boolean xoa = menu.DeleteThucDon(ma);
                if (xoa == true) {
                    FillTable(null);//load để cập nhật dữ liệu
                    try{
                        jpThucDon.td.FillLoai();// cập nhật menu 
                        jpThucDon.td.updateUI();// update giao diện
                    }catch(Exception e){
                        JOptionPane.showMessageDialog(null, "Xóa " +sp+" thành công ");
                    }
                }else
    ...
    private void txttimKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txttimKeyReleased
        ...
        Vector<ThucDon> arrTable = menu.SearchMon(txttim.getText());
        if(arrTable != null){
            DefaultTableModel tbmodel = new DefaultTableModel();
