...
public final class jpBanHang extends javax.swing.JPanel {
    ...
    public jpBanHang() {
        initComponents();
        bh = this;//gán giá trị của class jpBanHang = gtri hiện tại
        FillBan();
    ...
    public void FillBan(){
        ...
        arrBan = getinfoban.GetBan(0)   ;
        if(arrBan != null){
            jpBan.removeAll();
            JButton[] btn = new JButton[arrBan.size()];
            for(int i=0;i<arrBan.size();i++){
                btn[i] = new JButton();
                btn[i].setName(String.valueOf(arrBan.get(i).GetMaBan()));// k hiện lên như text
                String[] mb = arrBan.get(i).GetTenBan().split(" ");
                btn[i].setText(mb[1]);
                btn[i].setIcon(new javax.swing.ImageIcon(getClass().getResource("/Interface/Images/ico-Table.png")));// đường dẫn tương đối
                Border thickBorder = new LineBorder(Color.WHITE,5);// tạo đường viền, dộ dày dg viền
                btn[i].setBorder(thickBorder);
                btn[i].setBackground(Color.decode("#8080ff"));// set màu nền
                ...
                btn[i].addMouseListener(new MouseAdapter() {
                        @Override
                        public void mousePressed(MouseEvent e) {// click chuột nhất
                            JpGoiMon goimon;
                            ban = getinfoban.GetBan(Integer.parseInt(e.getComponent().getName()));//gán thông tin bàn đang click chuột vào theo mã bàn
                            if(ban != null){                            
                ...
                jpBan.add(btn[i]);
                jpBan.updateUI();// cập nhật giao diện bằng giá trị hiện tại 
            }
        }
